...
```{python}
grouped_pitches_by_outs = (
    filtered_pitchers.groupby(['player_name', 'pitch_type', 'outs_when_up'])
    .size()
    .reset_index(name='pitch_count')
)
grouped_pitches_by_outs['total_pitches_in_outs'] = (
    grouped_pitches_by_outs.groupby(['player_name', 'outs_when_up'])['pitch_count'].transform('sum')
)
grouped_pitches_by_outs['pitch_proportion'] = (
    grouped_pitches_by_outs['pitch_count'] / grouped_pitches_by_outs['total_pitches_in_outs']
)
...
including hierarchal clustering trees and heat maps. This proved to be
...
hierarchal clustering tree into 5 groups and viewed the pitch selection
...
pitching_data_for clustering = aggregated_pitchers_by_outs.drop(columns = ["player_name", "outs_when_up"])
...
```{python}
# Visualization of Fastball Selection By Cluster
pitch_selection_melted = pitch_selection.melt(
  id_vars = ["cluster"],
  var_name = "pitch_type",
...
pitch_selection_plot(
  ggplot(pitch_selection_melted, aes(x = "factor(cluster)", y = "proportion", fill = "pitch_type")) + geom_bar(stat = "identity", position = "dodge") +
  labs(
...
```{python}
# Heatmap of Pitch Groups
ranks_outs["last_name"] = ranks_outs["player_name"].str.extract(r"r^([^,]+)")
...
heatmap_plot = (
  ggplot(ranks_outs_long, aes(x = "outs_when_up", y = "last_name", fill = "rank")) + geom_tile() + 
  facet_wrap("~pitch_group", ncol = 1) + 
  scale_fill_gradient(low = "white", high = "red") +
  labs(
    title = "Heatmap of Pitch Group Ranks with Outs",
...
specific_counts(
  ...
  .size().reset_index(name = 'count_specific')
  .group_by(['player_name', 'count'])
...
count_prop = (
  ...
  .apply(lambda df: df.assign(total_pitches = len(df)))
  .reset_index(drop = True)
  .groupby(['player_name', 'pitch_group', 'count'])
  .apply(lambda df: pd.Series({'proportion: len(df) / df['total_pitches'].iloc[0]}))
  .reset_index()
)
# Add cluster column to comparison
comparison = comparison.merge(cluster_pitches_eight[['player_name', 'cluster']], on = 'player_name', how = 'left')
```

```{python}
count_diff = count_diff.drop(columns = ['proportion_specific', 'proportion_difference'])

avg_diff = (
  count_diff
  .groupby(['player_name', 'count', 'cluster'])
...
cluster_dropdown = widgets.Dropdown(options = avg_diff['cluster'].unique(), descirption = 'Cluster:')
```
...
```{python}
pitching23[['Firstname', 'Lastname']] = pitching23['Player'].str.split(' ', 1, expand = True)
pitching23['Player'] = pitching23['Lastname'] + ', ' + pitching23['Firstname']
pitching23['ERA'] = pitching23['ERA...3']
...
error_df = (
  ...
  .groupby(['player_name', 'count'])
  .agg(
    SSE=lambda x: (x['change_fastball']**2 + x['change_breaking_ball']**2 +           x['change_offspeed']**2).sum(),
    abs_sum=lambda x: (x['change_fastball'].abs() +                     x['change_breaking_ball'].abs() + x['change_offspeed'].abs()).sum(),
    ERA = ('ERA', 'first')
...
```{python}
so_pitchers = pitching23[['Player', 'SO']].copy()
avg_diff_so = avg_diff.merge(so_pitchers, left_on = 'player_name', right_on = 'Player', how = 'left').drop(columns = ['Player'])

error_df_so = (
  avg_diff_so
  .groupby(['player_name', 'count'])
  .agg(
      SSE=lambda x: ((x['change_fastball']**2 + x['change_breaking_ball']**2 + x['change_offspeed']**2).sum())**0.5,
      abs_sum=lambda x: (x['change_fastball'].abs() + x['change_breaking_ball'].abs() + x['change_offspeed'].abs()).sum(),
      SO = ('SO', 'first')
  )
  .reset_index()
...
stat_df['log_SSE'] = np.log(stat_df['SSE'])
